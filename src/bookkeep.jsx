...
const EXPENSE_CATEGORIES = {
  ...
  "Payment":["Shopping","Recreation","Medicine","Payment","Accommodation"],
};
const INCOME_CATEGORIES = { "Earnings":["Salary","Bonus"] };
...
function generateDemoData(){
  ...
  for(let m=0;m<12;m++){
    ...
    for(let d=1;d<=Math.min(dim,28);d++){
      ...
      if(d%7===0) txs.push({id:uid(),date:ds,type:"expense",category:"Payment",subcategory:"Shopping",description:["Groceries","Clothes","Electronics","Books"][d%4],amount:200+Math.floor(Math.random()*800),fundSource:"income"});
      if(d===15) txs.push({id:uid(),date:ds,type:"expense",category:"Payment",subcategory:"Accommodation",description:"Monthly rent",amount:8000+Math.floor(Math.random()*2000),fundSource:"income"});
      if(d===10) txs.push({id:uid(),date:ds,type:"expense",category:"Payment",subcategory:"Recreation",description:["Movie","Gym","Park","Concert"][m%4],amount:200+Math.floor(Math.random()*300),fundSource:"income"});
      if(d===20) txs.push({id:uid(),date:ds,type:"expense",category:"Payment",subcategory:"Medicine",description:"Pharmacy",amount:100+Math.floor(Math.random()*300),fundSource:"income"});
      // Some expenses from savings
      if(d===22&&m%2===0) txs.push({id:uid(),date:ds,type:"expense",category:"Payment",subcategory:"Shopping",description:"Big purchase (from savings)",amount:2000+Math.floor(Math.random()*3000),fundSource:"savings"});
    }
  }
...
// ─── API (Supabase via Netlify Functions) ───
...
// ─── Password Lock ───
// Password is hashed and stored in localStorage. The hash is NOT reversible.
function hashPw(pw){let h=0;const s=pw+"bk-salt-x7k";for(let i=0;i<s.length;i++){h=((h<<5)-h)+s.charCodeAt(i);h=h&h;}return"bk_"+Math.abs(h).toString(36);}
const PW_KEY="bookkeep_pw";
const SESSION_KEY="bookkeep_session";
const AUTO_LOCK_MS=30*60*1000; // 30 min
...
function PasswordScreen({onUnlock}){
  ...
  const doSetup=()=>{
    ...
    setPw2("");setMode("confirm");setError("");
  };
  const doConfirm=()=>{
    if(pw2!==pw){setError("Passwords do not match");setPw2("");return;}
    localStorage.setItem(PW_KEY,hashPw(pw));
    onUnlock();
  };
  const doLogin=()=>{
    if(lockSec>0) return;
    if(hashPw(pw)===localStorage.getItem(PW_KEY)){onUnlock();}
    else{
      const a=attempts+1;setAttempts(a);setPw("");
      if(a>=5){setLockSec(30);setError("Too many attempts. Wait 30 seconds.");setAttempts(0);}
      else setError(`Wrong password (${5-a} attempts left)`);
    }
  };

  const boxStyle={width:"100%",maxWidth:300,padding:"14px 18px",borderRadius:12,border:"2px solid rgba(255,255,255,.12)",background:"rgba(255,255,255,.06)",color:"#e8e8f0",fontSize:20,textAlign:"center",letterSpacing:4,outline:"none",fontFamily:"'JetBrains Mono',monospace",boxSizing:"border-box"};
  const btnOk={width:"100%",maxWidth:300,padding:14,borderRadius:12,border:"none",background:"linear-gradient(135deg,#ff6b6b,#ee5a24)",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",marginTop:14};
  const btnOff={...btnOk,background:"#333",cursor:"not-allowed"};

  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0c0c1d,#16213e,#0c0c1d)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Bricolage Grotesque',sans-serif",padding:20}}>
      ...
      <div style={{textAlign:"center",maxWidth:380,width:"100%"}}>
        ...
        <div style={{fontWeight:800,fontSize:28,letterSpacing:-1,background:"linear-gradient(135deg,#ff6b6b,#feca57)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:8}}>BookKeep</div>

        {mode==="loading"&&<div style={{color:"#8892b0",marginTop:20}}>Loading...</div>}

        {mode==="setup"&&<>
          <p style={{color:"#8892b0",fontSize:14,marginBottom:24}}>Create a password to protect your data</p>
          <input type="password" placeholder="New password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doSetup()} style={boxStyle} autoFocus/>
          <button onClick={doSetup} style={pw.length>=4?btnOk:btnOff}>Continue →</button>
          <div style={{fontSize:11,color:"#555",marginTop:14}}>Minimum 4 characters</div>
        </>}

        {mode==="confirm"&&<>
          <p style={{color:"#8892b0",fontSize:14,marginBottom:24}}>Confirm your password</p>
          <input type="password" placeholder="Re-enter password" value={pw2} onChange={e=>setPw2(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doConfirm()} style={boxStyle} autoFocus/>
          <button onClick={doConfirm} style={pw2?btnOk:btnOff}>Set Password ✓</button>
          <button onClick={()=>{setMode("setup");setPw("");setError("");}} style={{background:"transparent",border:"none",color:"#8892b0",cursor:"pointer",marginTop:12,fontSize:13}}>← Back</button>
        </>}

        {mode==="login"&&<>
          <p style={{color:"#8892b0",fontSize:14,marginBottom:24}}>Enter password to continue</p>
          <input type="password" placeholder="••••" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...boxStyle,borderColor:error?"#ff5252":"rgba(255,255,255,.12)"}} autoFocus disabled={lockSec>0}/>
          <button onClick={doLogin} style={(!pw||lockSec>0)?btnOff:btnOk}>{lockSec>0?`Locked (${lockSec}s)`:"Unlock 🔓"}</button>
        </>}

        {error&&<div style={{marginTop:14,padding:"10px 16px",borderRadius:10,background:"rgba(255,82,82,.12)",border:"1px solid rgba(255,82,82,.25)",color:"#ff5252",fontSize:13,fontWeight:600}}>{error}</div>}
      </div>
    </div>
  );
}
...
export default function BookKeep(){
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(()=>{if(Date.now()-lastActivity>AUTO_LOCK_MS) setAuthed(false);},15000);
    return()=>{window.removeEventListener("click",track);window.removeEventListener("keydown",track);clearInterval(iv);};
  },[authed,lastActivity]);
  ...
  // Password gate — show lock screen if not authenticated
  if(!authed) return <PasswordScreen onUnlock={()=>{setAuthed(true);setLastActivity(Date.now());}}/>;
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Bricolage Grotesque','Segoe UI',sans-serif",transition:"background .3s,color .3s"}}>
      ...
      <nav style={{background:T.navBg,backdropFilter:"blur(16px)",borderBottom:`1px solid ${T.navBorder}`,padding:"11px 16px",position:"sticky",top:0,zIndex:20}}>
        <div style={{maxWidth:960,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:3,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <button onClick={()=>setAuthed(false)} title="Lock app" style={{padding:"7px 11px",borderRadius:24,border:`1px solid ${T.border}`,background:"transparent",color:T.textMuted,cursor:"pointer",fontSize:13}}>🔒</button>
          </div>
        </div>
      </nav>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"20px 16px"}}>
        ...
        {view==="report"&&<>
          ...
          <div style={{textAlign:"center",marginTop:30,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
            <button onClick={()=>{if(confirm("Change your password? You will set a new one.")){localStorage.removeItem(PW_KEY);setAuthed(false);}}} style={{background:"transparent",border:`1px solid ${T.border}`,color:T.textMuted,fontSize:12,cursor:"pointer",padding:"8px 18px",borderRadius:8}}>🔑 Change Password</button>
            <div style={{fontSize:10,color:T.textDim}}>Auto-locks after 30 minutes of inactivity</div>
