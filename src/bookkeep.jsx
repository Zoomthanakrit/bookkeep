...
export default function BookKeep(){
  ...
  useEffect(()=>{apiLoad().then(txs=>{if(txs.length>0){setTransactions(txs);}else{setTransactions(generateDemoData());}setLoading(false);}).catch(()=>setLoading(false));},[]);
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(editId){
        const updated=await apiUpdate(editId,tx);
        persist(transactions.map(t=>t.id===editId?updated:t));
        setEditId(null);showToast("Updated!");
      }else{
        const saved=await apiAdd(tx);
        persist([saved,...transactions]);showToast("Saved!");
      }
    }catch(e){showToast("Failed to save — check connection",true);return;}
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"20px 16px"}}>
        ...
        {view==="report"&&<>
          ...
          <div style={{textAlign:"center",marginTop:30,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
            <button onClick={()=>{if(confirm("Reset ALL data? This only resets your local view. Database data remains.")){const d=generateDemoData();persist(d);showToast("View reset!");}}} style={{background:"transparent",border:"none",color:T.textDim,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Reset to demo data</button>
          </div>
        </>}
